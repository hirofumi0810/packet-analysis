...
def plot_source_port_var(dirname, filename):
    """
    source ポート番号の分散のグラフをプロット
    """
    ...
    source_port_var = [float(line.split(",")[3]) for line in lines]

    # グラフプロット
    plt.clf()
    plt.plot(elapsed_time, source_port_var, color='b')
    ...
    plt.grid(True)
    plt.title('Variance of source port (' + filename.split(".")[0] + ')')
    plt.savefig("graph/source_port_var/source_port_var_" +
                filename.split(".")[0] + ".png", format='png', dpi=200)
    # plt.show()


def hist_source_port_var(dirname, filename):
    """
    source ポート番号の分散のヒストグラムをプロット
    """
    ...
    # ヒストグラム
    plt.clf()
    plt.hist(source_port_var, bins=40, color='b')
    plt.xlabel('Variance', size=15)
    plt.ylabel('Frame Number', size=15)
    plt.grid(True)
    plt.title('Histgram of variance of source port (' +
              filename.split(".")[0] + ')')
    plt.savefig("graph/hist_source_port_var/hist_source_port_var_" +
                filename.split(".")[0] + ".png", format='png', dpi=200)
    # plt.show()
...
if __name__ == "__main__":
    ...
    for i in range(len(filename)):
        if filename[i] != ".DS_Store":
            plot_source_port_var(dirname, filename[i])
            hist_source_port_var(dirname, filename[i])
